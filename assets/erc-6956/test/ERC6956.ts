...
testCases.forEach( ({upgradeable, description}) => {
  describe(`ERC6956${description}: Asset-Bound NFT --- Basics`, function () {
    ...
    async function deployAbNftFixture() {
      ...
      if(upgradeable) {
        ...
        const plainContract = await AbNftContract.connect(owner).deploy("Asset-Bound NFT test", "ABNFT");
  ...
  describe("Authorization Map tests", function () {
    ...
    describe("ERC721Burnable-compatible behavior", function () {
      ...
      it("SHOULD allow issuer to burn", async function() {
        ...
        // Maintainer is the issuer, so should be allowed to burn
